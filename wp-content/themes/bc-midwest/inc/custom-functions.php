...
/*Create custom field create_mobile_text for pages, post, custom posts */
add_action( 'add_meta_boxes', 'bc_create_mobile_text_metabox' );
function bc_create_mobile_text_metabox() {
global $post;
    // if ( is_front_page() ) {
        add_meta_box(
            'bc_mobile_text',
...
function bc_mobile_text() {
global $post;
$title = get_post_meta( $post->ID, 'home_mobile_text_overlay', true );?>
<textarea class="form-control" rows="6" cols="85" name="bc_mobile_text" id="title"><?= $title ?></textarea>
<?php
wp_nonce_field( 'bc_mobile_text_overlay_metabox_nonce', 'bc_mobile_text_metabox_process' );
}
/** Save the metabox */
add_action( 'save_post', 'bc_save_mobile_text_overlay', 1, 2 );
function bc_save_mobile_text_overlay( $post_id, $post ) {
    if ( !isset( $_POST['bc_mobile_text_metabox_process'] ) ) return;
    if ( !wp_verify_nonce( $_POST['bc_mobile_text_metabox_process'], 'bc_mobile_text_overlay_metabox_nonce' ) ) {
        return $post->ID;
    }
    if ( !current_user_can( 'edit_post', $post->ID )) {
        return $post->ID;
    }
    if ( !isset( $_POST['bc_mobile_text'] ) ) {
        return $post->ID;
    }
    $sanitizedtitle = wp_filter_post_kses( $_POST['bc_mobile_text'] );
    update_post_meta( $post->ID, 'home_mobile_text_overlay', $sanitizedtitle );
}

/*Create custom field Heading text for pages, post, custom posts */
/*add_action( 'add_meta_boxes', 'bc_create_heading_text_metabox' );
function bc_create_heading_text_metabox() {
global $post;
    // if ( is_front_page() ) {
        add_meta_box(
            'bc_heading_text',
...
function bc_heading_text() {
global $post;
$title = get_post_meta( $post->ID, 'home_heading_text_overlay', true );?>
<textarea class="form-control" rows="6" cols="85" name="bc_heading_text" id="title"><?= $title ?></textarea>
<?php
wp_nonce_field( 'bc_heading_text_overlay_metabox_nonce', 'bc_heading_text_metabox_process' );
}
/** Save the metabox */
// add_action( 'save_post', 'bc_save_heading_text_overlay', 1, 2 );
// function bc_save_heading_text_overlay( $post_id, $post ) {
//     if ( !isset( $_POST['bc_heading_text_metabox_process'] ) ) return;
//     if ( !wp_verify_nonce( $_POST['bc_heading_text_metabox_process'], 'bc_heading_text_overlay_metabox_nonce' ) ) {
//         return $post->ID;
//     }
//     if ( !current_user_can( 'edit_post', $post->ID )) {
//         return $post->ID;
//     }
//     if ( !isset( $_POST['bc_heading_text'] ) ) {
//         return $post->ID;
//     }
//     $sanitizedtitle = wp_filter_post_kses( $_POST['bc_heading_text'] );
//     update_post_meta( $post->ID, 'home_heading_text_overlay', $sanitizedtitle );
// }
    ...
    <?php
        ...
        array_multisort($order, SORT_ASC, $data);
            foreach ($data as $key => $value) {
                if(!empty($value->icon)){
                ?>
                <!--   <a arget="_blank" title="<?php echo $value->name;?>" href="<?php echo $value->url;?>">
                  <span class="bc_text_16 bc_line_height_32 ml-1 bc_color_warning bc_text_bold text-uppercase"><i class="<?php echo $value->icon;?>"></i></span></a> -->


                <a target="_blank" class="bc_text_16 bc_line_height_32 ml-1 bc_color_warning bc_text_bold text-uppercase no_hover_underline" title="<?php echo $value->name;?>" href="<?php echo $value->url;?>">
                    <i class="bc_text_26 bc_color_secondary_hover  bc_color_primary <?php echo $value->icon;?>"></i>
                </a>    
            <?php } 
            }
        }
